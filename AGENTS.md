...
- `recipes/opencode/`: the current package builds the CLI from source with Bun, following upstream's `packages/opencode/script/build.ts --single --skip-install` flow. Keep the Bun version-relaxation patch unless `conda-forge` catches up to upstream's required Bun. The recipe relies on a vendored `node_modules` source artifact plus a pinned `models.dev` API snapshot source, so do not reintroduce `bun install` during `conda-build`. When refreshing dependencies, mirror upstream's Nix workflow (`bun install --filter ... --ignore-scripts`, then `nix/scripts/canonicalize-node-modules.ts` and `nix/scripts/normalize-bun-binaries.ts`) before regenerating the vendored archive. Keep runtime library constraints aligned with Bun's linked ABI, and keep tests to non-network CLI paths like `--version`, `--help`, and `completion`.
